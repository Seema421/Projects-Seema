...
const { blogsV } = require("../models/blogsValidation")
const { result } = require("@hapi/joi/lib/base")

// blog validation using joi
const createValidatedBlogs = async function (req, res) {
    try {
        let data = req.body
        let result = await blogsV.validateAsync(data)
...
module.exports.createValidatedBlogs = createValidatedBlogs
...
const updateBlogs = async function (req, res) {
    try {
        ...
        if(updatedblog==null) return res.status(404).send({msg:"Data already deleted can not modify"})
...
const deletedBlog = async function (req, res) {
    try {
        ...
        if(deletedBlog==null) return res.status(404).send({msg:"Data already deleted can not modify"})
